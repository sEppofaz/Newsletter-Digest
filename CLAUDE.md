...
- **Pull-to-Refresh-touchend prüfte keine Zugdistanz (behoben 2026-07-24, Bug seit initialem Commit):** `touchend`-Handler löste bei JEDEM Tap oben auf der Seite (u.a. jeder Tab-Klick) einen vollen Refresh inkl. `renderTabs()` aus – nur `touchmove` prüfte die 50px-Schwelle (nur für den visuellen Indikator). Symptom: Tab-Wechsel (z.B. zu „Finanzen") sprang sofort zurück zu Tab 0 („KI & Tech"), da `renderTabs()` immer Kategorie 0 aktiv setzt. Fix: `touchend` prüft jetzt zusätzlich `ptrEl.classList.contains('show')` (also ob tatsächlich über die Schwelle gezogen wurde), bevor refresht wird.
