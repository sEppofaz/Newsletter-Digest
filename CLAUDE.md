...
- **Bulk-IMAP-Operationen (>100 Mails) über Sequenznummern sind unzuverlässig:** Beim einmaligen Rückstau-Cleanup (2026-07-24, 310 Mails) brach ein Sequenznummer-basierter `for mid in seen_ids: copy(); store(+Deleted)`-Loop + einmaligem `expunge()` am Ende nach 310 gemeldeten Erfolgen (0 Fehler) nur ~160 Mails wirklich ab – Ursache nicht abschließend geklärt (vermutlich Sequenznummer-Drift bei sehr vielen Operationen in einer Session). Fix: UID-basierte Operationen (`imap.uid('SEARCH', ...)`, `imap.uid('COPY', uid, ...)`, `imap.uid('STORE', uid, '+FLAGS', '(\\Deleted)')`) – UIDs sind stabil und ändern sich nie, im Gegensatz zu Sequenznummern. Bei künftigen Bulk-Operationen (>50 Mails) immer UID-basiert arbeiten, nie Sequenznummern. **Nachtrag 2026-09-25:** Diese Regel war bis dahin nur in `archive_known_senders()` umgesetzt – `fetch_mails()` und `fetch_from_all_mail()` liefen weiter über Sequenznummern. In `fetch_mails()` war das der gefährlichere Fall: Die Archivierungs-Schleife setzt `\\Deleted`, und bei einem Expunge während des Laufs (Handy, Serverfilter) hätte sie das auf der falschen Mail getan. Jetzt sind alle drei Funktionen UID-basiert. Lehre: Eine so gelernte Regel gleich an **allen** Stellen durchziehen, nicht nur an der, wo sie aufgeschlagen ist.
- **FETCH-Response nie als `data[0][1]` auswerten (gefixt 2026-09-25, aufgeschlagen im QG-Feedback-Bot):** `imap.fetch()` liefert eine Liste, deren Elemente entweder das erwartete Tupel `(b'12 (RFC822 {n}', b'<rohe Mail>')` oder ein reines Bytes-Objekt sind – Gmail streut jederzeit *unsolicited untagged responses* ein (`* 12 FETCH (FLAGS (\\Seen))`), z.B. wenn parallel eine Mail am Handy geöffnet wird. Dann ist `data[0][1]` **ein `int`** (Byte-Indexing!) → `email.message_from_bytes(int)` wirft `'int' object has no attribute 'decode'`. Die Wache `if not data[0]` greift nicht, weil das Bytes-Objekt nicht leer ist. Im QG-Feedback-Bot ist das zweimal live aufgeschlagen (2026-09-17/19), hier war es an allen drei FETCH-Stellen latent vorhanden. Fix: `raw_from_fetch()` sucht das erste echte Tupel und überspringt Bytes-Elemente. Grundmuster: `PKA/BKM/IMAP-Polling.md`.
- **Traceback im Journal:** Die generischen Except-Blöcke nutzen seit 2026-09-25 `log.exception` statt `log.error` – ohne Traceback war bei `'int' object has no attribute 'decode'` nicht erkennbar, welche `.decode()`-Stelle gemeint war.
...
- **Gmail-App-Passwort kann ohne Vorwarnung ablaufen/widerrufen werden:** Löste am 2026-08-19 einen `AUTHENTICATIONFAILED`-Fehler aus, der bis 2026-09-01 unbemerkt blieb. Symptom: `newsletter-fetch.timer` läuft „erfolgreich" durch (Exit 0), aber es entstehen keine Digests mehr. Neues Passwort unter https://myaccount.google.com/apppasswords generieren, in `/opt/newsletter-digest/.env` (`GMAIL_APP_PASSWORD`) eintragen.
- **Telegram-Sendefunktionen prüften bis 2026-09-01 weder HTTP-Status noch loggten sie Fehler bei ungültigem Bot-Token (`notify_telegram()` in `fetch_mails.py`, `_send_telegram_raw()` in `app.py`):** `requests.post()` wirft bei 401/403/etc. **keine** Exception, ein `except Exception` fängt das also nicht ab – Antwort wurde nie geprüft (`r.ok`). Genau das passierte parallel zum Gmail-App-Passwort-Ausfall: der Telegram-Bot-Token für dieses Projekt war ebenfalls ungültig geworden (401 Unauthorized, vermutlich gleicher Zeitraum ~18./19.08.), wodurch **weder** Fehler-Alerts **noch** die normalen Digest-Teaser ankamen – komplett unsichtbar, auch in `journalctl`, da `notify_telegram()` in `fetch_mails.py` den Fehler zusätzlich mit `except Exception: pass` schluckte (kein Log-Eintrag). Fix (2026-09-01): beide Funktionen prüfen jetzt `r.ok` und loggen den HTTP-Status bei Fehlschlag. Neuen Bot-Token via @BotFather generieren (falls der alte Bot gelöscht/zurückgesetzt wurde) und in `.env` (`TELEGRAM_BOT_TOKEN`) eintragen. **Lehre für andere Projekte:** Jede Telegram-Sendefunktion sollte `r.ok` prüfen und bei Fehlschlag loggen – ein stiller Fehlschlag bei der eigenen Alarmierungsfunktion ist der gefährlichste Fehlerfall überhaupt (man verlässt sich gerade dann auf den Alarm, wenn etwas kaputt ist).
